...
def showImage(title = "Original Image", img = image):
...
def sepia(image = image, filter_degre = [[0.272, 0.534, 0.131],[0.349, 0.686, 0.168],[0.393, 0.769, 0.189]]):
...
def blur(image = image,size=(15,15)):
...
def pixelation(image = image,pixel_size = 10):
...
def edge_detection(image = image,threshold = [50,150]):
  edges = cv2.Canny(image,threshold[0],threshold[1])
  # showImage("Edge detection effect",edges)
  return edges

def emboss(image = image,kernel = [[ -2, -1,  0],[ -1,  1,  1],[  0,  1,  2]]):
